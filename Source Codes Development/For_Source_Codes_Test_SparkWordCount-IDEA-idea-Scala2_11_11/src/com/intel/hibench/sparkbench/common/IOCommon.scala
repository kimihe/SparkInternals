...
class IOCommon(val sc:SparkContext) {
  def load[T:ClassTag:TypeTag](filename:String, force_format:Option[String]=None) = {
    ...
    val input_format = force_format.getOrElse("Text")
  ...
  def save(filename:String, data:RDD[_], prefix:String) = {
    ...
    output_format match {
      case "Text" =>
        //        if (output_format_codec.)  data.saveAsTextFile(filename)
        //        else data.saveAsTextFile(filename, output_format_codec)

        data.saveAsTextFile(filename)

      case "Sequence" =>
        val sequence_data = data.map(x => (NullWritable.get(), new Text(x.toString)))
        //        if (output_format_codec) {
